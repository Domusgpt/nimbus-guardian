...
### Session 34 – History window filters
- **Date**: 2025-10-10 (UTC)
- **Completed**
  - Added `since` filtering to the observability exporter and dashboard API so operators can request history slices starting at a specific timestamp.
  - Extended the observability status CLI and client helper to pass the new filter, updated documentation, and captured regression tests for exporter, client, and server plumbing.
- **Tests**: `npm test`
