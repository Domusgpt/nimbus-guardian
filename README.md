...
Nimbus Guardian’s public site now ships with a holographic **Observability Command Center** section that keeps the existing neon-grid vibe intact while making the Claude Code bundle impossible to miss. The refreshed module:

- Surfaces the full `/observability-*` command family with mirrored CLI examples so operators can jump from browser to terminal without context switching.
- Points directly to the Claude marketplace bundle, adds a parserstor.com CTA, and teases the soon-to-launch **Reposiologist** and **Vib3-Scribe** copilots as first-class cards without introducing any off-brand styling.
- Reuses the current gradients, typography, and particle system so the site’s holographic voice stays untouched—only the content tiles are new.

The documentation portal (`DOCS_SITE_PLAN.md`) tracks the follow-up work so the docs microsite can mirror the Observability Hub content without breaking the shared aesthetic.
