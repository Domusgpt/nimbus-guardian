...
The log writer creates directories as needed, stores entries with a `receivedAt` timestamp, and automatically rotates the file once it grows past ~1 MB (keeping up to five historical archives with `.1`, `.2`, etc. suffixes).
...
Each request includes the fingerprint summary plus a `sentAt` timestamp so downstream processors can reconcile delivery time. Headers are loaded from the JSON blob exactly as provided, making it easy to attach API tokens or routing hints without touching code.
