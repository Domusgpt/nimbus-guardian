...
When `--history` is supplied Nimbus appends a “Recent observability history” section after the live snapshot (or returns `{ "snapshot": ..., "history": [...] }` in JSON mode) so operators can see the latest fingerprint totals and sink counts over the requested window.
