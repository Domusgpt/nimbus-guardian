...
Need to focus on a specific window? Append a `since` filter to retrieve only the snapshots emitted at or after a given timestamp (ISO 8601 or millisecond epoch values are supported):

```
GET /api/observability/history?since=2025-01-01T00:00:00Z&limit=25
```

The exporter trims older entries server-side so you can zoom in on recent incidents without pulling the entire buffer across the wire.
...
When `--history` is supplied Nimbus appends a “Recent observability history” section after the live snapshot (or returns `{ "snapshot": ..., "history": [...], "overview": {...} }` in JSON mode) so operators can see the latest fingerprint totals and sink counts over the requested window. Pair the flag with `--since` to focus on a particular timeframe (for example, everything emitted after a maintenance window opened) without downloading older entries. Nimbus also prints an aggregated history overview ahead of the timestamped entries so you can glance at coverage length, min/max fingerprint totals, average activity, and sink transition counts without parsing the raw payloads.
