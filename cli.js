    ...
    .action(async (options) => {
        ...
        try {
            ...
            if (includeHistory) {
                ...
                history = await client.fetchHistory({ cookie: options.cookie, limit: includeHistory });
            }
            ...
            if (options.json) {
                if (includeHistory) {
                    console.log(JSON.stringify({ snapshot, history }, null, 2));
                } else {
                ...
                if (includeHistory && history && history.length > 0) {
                    ...
                    history.forEach((entry, index) => {
                        const timestamp = entry?.fingerprints?.generatedAt
                            || entry?.emittedAt
                            || 'unknown';
                        const total = entry?.fingerprints?.totalTracked;
                        const sinks = Array.isArray(entry?.sinks) ? entry.sinks.length : 0;
                        const label = `#${index + 1}`;
