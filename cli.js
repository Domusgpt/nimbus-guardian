...
program
    ...
    .option('--since <timestamp>', 'Only include history snapshots emitted at or after <timestamp> (ISO date or milliseconds)')
    ...
    .action(async (options) => {
        ...
        if (typeof options.since === 'string') {
            ...
            if (trimmed.length > 0) {
                historySince = trimmed;
            }
        }

        const shouldFetchHistory = includeHistory > 0 || !!historySince;
        ...
        try {
            ...
            if (shouldFetchHistory) {
                ...
                historyResult = await client.fetchHistory({
                    cookie: options.cookie,
                    limit: includeHistory > 0 ? includeHistory : null,
                    since: historySince
                });
            }
            ...
            if (!snapshot) {
                ...
                if (shouldFetchHistory && historyEntries.length > 0) {
                    if (historyOverview) {
                    ...
                    console.log(chalk.gray('\nNo observability history available for the requested window.'));
                }
                return;
            }
