...
const printHistoryEntries = (
    ...
    historyEntries.forEach((entry, index) => {
        ...
        if (typeof total === 'number') {
            console.log(chalk.gray(`${indent}total tracked: ${total}`));
        }
        console.log(chalk.gray(`${indent}sinks: ${sinks}`));
        ...
        if (Number.isFinite(totalDelta) && totalDelta !== 0) {
            ...
            console.log(chalk.gray(`${indent}Δ total tracked: ${deltaLabel}`));
        }
...
const renderObservabilitySnapshot = ({
    snapshot,
    historyEntries = [],
    historyOverview = null,
    includeHistory = false
}) => {
    const historyEnabled = Number.isFinite(includeHistory)
        ? includeHistory > 0
        : Boolean(includeHistory);

    const normalizedHistory = Array.isArray(historyEntries) ? historyEntries : [];
    const normalizedOverview = historyOverview && typeof historyOverview === 'object'
        ? historyOverview
        : null;
    ...
    if (!snapshot) {
        ...
        if (historyEnabled) {
            if (normalizedOverview) {
                printHistoryOverview(normalizedOverview);
            }
            printHistoryEntries(normalizedHistory);
        }
        return;
    }
...
program
    .command('observability-digest')
    .description('Capture snapshot, history, and incident summaries from the dashboard API')
    .option('--url <url>', 'Observability status URL', 'http://localhost:3333/api/observability')
    .option('--history <count>', 'Include the latest <count> observability snapshots in the digest (default: 5)', '5')
    .option('--incidents-limit <count>', 'Include up to <count> incident records in the digest (default: 5)', '5')
    .option('--cookie <value>', 'guardian_session cookie value or full Cookie header')
    .option('--header <header...>', 'Additional request header(s) in KEY:VALUE format')
    ...
    .option('--open', 'Only include open incidents in the digest')
    .option('--closed', 'Only include closed incidents in the digest')
    .option('--json', 'Print combined JSON payload instead of formatted summary')
    .option('--timeout <seconds>', 'Request timeout in seconds (default: 5)')
    .option('--history-url <url>', 'Override observability history URL')
    .option('--incidents-url <url>', 'Override observability incidents URL')
    .action(async (options) => {
        const headers = parseHeaderOptions(options.header, {
            onWarning: (message) => console.log(chalk.yellow(message))
        });
        ...
        const historyRaw = options.history ?? '5';
        const historyValue = Number.parseInt(historyRaw, 10);
        let historyLimit = Number.isFinite(historyValue) && historyValue >= 0
            ? Math.floor(historyValue)
            : 5;
        if (options.history !== undefined && (!Number.isFinite(historyValue) || historyValue < 0)) {
            console.log(chalk.yellow('Ignoring --history value; expected a non-negative integer.'));
        }

        const incidentsRaw = options.incidentsLimit ?? '5';
        const incidentsValue = Number.parseInt(incidentsRaw, 10);
        let incidentsLimit = Number.isFinite(incidentsValue) && incidentsValue >= 0
            ? Math.floor(incidentsValue)
            : 5;
        if (options.incidentsLimit !== undefined && (!Number.isFinite(incidentsValue) || incidentsValue < 0)) {
            console.log(chalk.yellow('Ignoring --incidents-limit value; expected a non-negative integer.'));
        }
        ...
        if (options.open && options.closed) {
            console.log(chalk.yellow('Both --open and --closed supplied; including all incidents.'));
        ...
        const client = new ObservabilityStatusClient({
            ...
            incidentsUrl: options.incidentsUrl,
            headers,
            timeoutMs,
            logger: console
        });

        const spinner = ora('Fetching observability digest...').start();

        try {
            spinner.text = 'Fetching observability snapshot...';
            const snapshot = await client.fetchStatus({ cookie: options.cookie });

            let historyResult = null;
            if (historyLimit > 0) {
                spinner.text = 'Fetching observability history...';
                historyResult = await client.fetchHistory({ cookie: options.cookie, limit: historyLimit });
            }

            spinner.text = 'Fetching observability incidents...';
            const incidentReport = await client.fetchIncidents({
                cookie: options.cookie,
            ...
            spinner.succeed('Observability digest retrieved.');
            ...
            const digest = {
                snapshot,
                history: historyLimit > 0 ? historyEntries : [],
                overview: historyLimit > 0 ? historyOverview : null,
            ...
            renderObservabilitySnapshot({
                snapshot,
                historyEntries: digest.history,
                historyOverview: digest.overview,
                includeHistory: historyLimit
            });

            printIncidentReport(incidentReport);
        } catch (error) {
            spinner.fail('Unable to fetch observability digest');
            console.error(chalk.red(error.message));
            process.exitCode = 1;
        } finally {
            if (spinner.isSpinning) {
