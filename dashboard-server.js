...
class DashboardServer {
    constructor(port = 3333, options = {}) {
        ...
        const sessionCallbacks = {
            onSessionCreated: sessionManagerConfig.onSessionCreated,
            onSessionTouched: sessionManagerConfig.onSessionTouched,
            onSessionDestroyed: sessionManagerConfig.onSessionDestroyed,
            onSessionExpired: sessionManagerConfig.onSessionExpired
        };

        this.sessionManager = options.sessionManager || new SessionManager({
            ...sessionManagerConfig,
            onSessionCreated: (session) => {
                this.observability.recordSessionCreated(session);
                this.recordFingerprintMetric('created', session);
                if (typeof sessionCallbacks.onSessionCreated === 'function') {
                    sessionCallbacks.onSessionCreated(session);
                }
            },
            onSessionTouched: (session) => {
                this.observability.recordSessionTouched(session);
                this.recordFingerprintMetric('touched', session);
                if (typeof sessionCallbacks.onSessionTouched === 'function') {
                    sessionCallbacks.onSessionTouched(session);
                }
            },
            onSessionDestroyed: (session) => {
                this.observability.recordSessionDestroyed(session);
                this.recordFingerprintMetric('ended', session);
                if (typeof sessionCallbacks.onSessionDestroyed === 'function') {
                    sessionCallbacks.onSessionDestroyed(session);
                }
            },
            onSessionExpired: (session) => {
                this.observability.recordSessionExpired(session);
                this.recordFingerprintMetric('ended', session);
                if (typeof sessionCallbacks.onSessionExpired === 'function') {
                    sessionCallbacks.onSessionExpired(session);
                }
            }
        });

        this.fingerprintStore = this.fingerprintStore || new FingerprintStore({
            basePath: this.projectPath,
            ttlMs: this.sessionManager.ttlMs,
            now: sessionManagerConfig.now
        });
        ...
        this.rateLimitProfiles = {
            action: {
                name: 'action',
                limiter: this.createRateLimiter('action', { windowMs: 60 * 1000, max: 40 }),
                message: 'Too many dashboard actions. Please slow down before trying again.'
            },
            scan: {
                name: 'scan',
                limiter: this.createRateLimiter('scan', { windowMs: 5 * 60 * 1000, max: 2 }),
                message: 'Security scans are throttled. Wait a moment before running another scan.'
            },
            chat: {
                name: 'chat',
                limiter: this.createRateLimiter('chat', { windowMs: 60 * 1000, max: 8 }),
                message: 'Too many chat requests in a short time. Pause briefly before asking again.'
            },
            install: {
                name: 'install',
                limiter: this.createRateLimiter('install', { windowMs: 10 * 60 * 1000, max: 3 }),
    ...
    createRateLimiter(profileName, config = {}) {
        if (this.options?.rateLimiterFactory) {
            return this.options.rateLimiterFactory(profileName, config, (event) => {
                this.observability.recordRateLimitAttempt(profileName, event);
            });
        }
    ...
    async loadConfig() {
        ...
        if (this.fingerprintStore && typeof this.fingerprintStore.ensureLoaded === 'function') {
            await this.fingerprintStore.ensureLoaded();
        }
    }

    getObservabilitySnapshot() {
        const snapshot = this.observability.getSnapshot();
    ...
    getObservabilityHistory(options = {}) {
        const { limit = null, since = null } = options || {};
        ...
        const normalizedSince = this._normalizeHistorySince(since);
    ...
    recordFingerprintMetric(eventType, session) {
        ...
        try {
            ...
            if (eventType === 'created') {
                task = this.fingerprintStore.recordSessionCreated(fingerprint, payload);
            } else if (eventType === 'touched') {
                task = this.fingerprintStore.recordSessionTouched(fingerprint, payload);
            } else {
    ...
    recordFingerprintMetadata(session, fingerprint) {
        if (!this.fingerprintStore || !session || !fingerprint) {
            return;
        }
    ...
    establishBrowserSession(req, res, options = {}) {
        ...
        if (!session) {
            ...
            if (fingerprint) {
                this.recordFingerprintMetadata(session, fingerprint);
            }
        ...
        try {
            ...
            if (url.pathname === '/api/observability/history' && req.method === 'GET') {
                ...
                const since = typeof sinceParam === 'string' && sinceParam.trim().length > 0
                    ? sinceParam.trim()
                    : null;
                const { history, overview } = this.getObservabilityHistory({ limit, since });
                res.writeHead(200);
            ...
            if (url.pathname === '/api/scan' && req.method === 'POST') {
                const engine = this.createGuardianEngine();
            ...
            if (url.pathname === '/api/tools' && req.method === 'GET') {
                const detector = this.createToolDetector();
            ...
            if (url.pathname === '/api/chat' && req.method === 'POST') {
                ...
                const ai = this.createAIAssistant();
    ...
    async getProjectStatus() {
        const status = {
            ...
            observability: this.getObservabilitySnapshot()
        };
    ...
    createGuardianEngine() {
        return new GuardianEngine(this.projectPath, this.config);
    }
    ...
    createToolDetector() {
        return new ToolDetector(this.projectPath);
    }
...
<body>
    ...
    <script nonce="${nonce}">
        ...
        const formatRateLimitLabel = (value) => {
            if (!value) {
                return '';
            }
        ...
        async function loadProjectStatus() {
            try {
                ...
                if (data.observability?.sessions) {
                    const sessions = data.observability.sessions;
                    const active = escapeHtml(sessions.active ?? 0);
                    const touches = escapeHtml(sessions.touches ?? 0);
                    const created = escapeHtml(sessions.created ?? 0);
                    sections.push(
                        '<div class="metric">' +
                        ...
                        '</div>' +
                        '<div class="metric">' +
                            '<span class="metric-label">Session Touches</span>' +
                            '<span class="metric-value">' + touches + '</span>' +
                        '</div>' +
                        '<div class="metric">' +
                            '<span class="metric-label">Sessions Created</span>' +
                            '<span class="metric-value">' + created + '</span>' +
                        '</div>'
                    );
                }

                if (data.observability?.rateLimits) {
                    const entries = Object.entries(data.observability.rateLimits);
                    entries.forEach(([profile, info]) => {
                        const formattedName = formatRateLimitLabel(profile) || 'Rate Limit';
                        const labelText = formattedName + ' Blocks';
                        const label = escapeHtml(labelText);
                        const blocked = escapeHtml(info.blocked ?? 0);
                        sections.push(
                            '<div class="metric">' +
