...
class ObservabilityStatusClient {
    ...
    async fetchHistory(options = {}) {
        ...
        const { cookie = null, headers = {}, limit = null, since = null } = options;
        ...
        try {
            ...
            const sinceParam = this._normalizeHistorySinceParam(since);
            if (sinceParam) {
                parsed.searchParams.set('since', sinceParam);
            }
            ...
            if (sinceParam) {
                const separator = requestUrl.includes('?') ? '&' : '?';
                requestUrl = `${requestUrl}${separator}since=${encodeURIComponent(sinceParam)}`;
            }
        }
    ...
    _normalizeHistorySinceParam(value) {
