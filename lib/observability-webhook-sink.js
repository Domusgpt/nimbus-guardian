...
class ObservabilityWebhookSink {
    ...
    async flush() {
        ...
        return this._flushQueue('manual');
    }
    ...
    _enqueue(payload) {
        ...
        if (this.batchMaxItems === 1 && this.batchMaxWaitMs === 0 && this.batchMaxBytes === 0) {
            this.queueBytes = 0;
            this._recordBatchingEvent('immediate', [entry]);
        ...
        if (this.queue.length >= this.batchMaxItems) {
            this._flushQueue('max-items').catch(() => {});
        } else if (this.batchMaxBytes > 0 && this.queueBytes >= this.batchMaxBytes) {
            this._flushQueue('max-bytes').catch(() => {});
        } else {
    ...
    _ensureFlushTimer() {
        ...
        this.flushTimer = setTimeout(() => {
            this.flushTimer = null;
            this._flushQueue('max-wait').catch(() => {});
        }, this.batchMaxWaitMs);
        this.flushTimer.unref?.();
    }

    _flushQueue(reason = 'manual') {
        if (this.queue.length === 0) {
        ...
        this._recordBatchingEvent(reason, entries);
    ...
    getTelemetry() {
        return {
            ...
            batchingStats: { ...this.batchingStats },
            queue: {
    ...
    _recordBatchingEvent(reason, entries) {
        if (!Array.isArray(entries) || entries.length === 0) {
            return;
        }

        const timestamp = this._timestamp();
        const totalBytes = entries.reduce((acc, entry) => acc + (Number(entry.size) || 0), 0);

        if (reason === 'immediate') {
            this.batchingStats.immediateSends += 1;
            this.batchingStats.lastImmediateSendAt = timestamp;
        } else {
            this.batchingStats.totalFlushes += 1;
            this.batchingStats.lastFlushAt = timestamp;
            this.batchingStats.lastFlushReason = reason;

            switch (reason) {
                case 'manual':
                    this.batchingStats.manualFlushes += 1;
                    break;
                case 'max-items':
                    this.batchingStats.maxItemsFlushes += 1;
                    break;
                case 'max-bytes':
                    this.batchingStats.maxBytesFlushes += 1;
                    break;
                case 'max-wait':
                    this.batchingStats.maxWaitFlushes += 1;
                    break;
                case 'drain':
                    this.batchingStats.drainFlushes += 1;
                    break;
                default:
                    break;
            }
        ...
        if (entries.length > this.batchingStats.largestBatchSize) {
            this.batchingStats.largestBatchSize = entries.length;
        }

        if (totalBytes > this.batchingStats.largestBatchBytes) {
            this.batchingStats.largestBatchBytes = totalBytes;
        }
    ...
    _notifyTelemetryChange() {
        if (!this.onTelemetry || this.telemetryScheduled) {
            return;
        }
        ...
        queueMicrotask(() => {
            ...
            try {
                ...
                this.logger?.warn?.('Observability webhook telemetry callback failed', error);
            }
        });
    }
}
