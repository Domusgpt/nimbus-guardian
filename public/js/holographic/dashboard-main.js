...
createClipboardDelegate({
    ...
    onCopyStart: ({ trigger }) => {
        if (!trigger) return;
        const timer = clipboardResetTimers.get(trigger);
        if (timer) {
            clearTimeout(timer);
            clipboardResetTimers.delete(trigger);
        }
        trigger.classList.remove('is-copied', 'is-error');
    },
    onCopySuccess: ({ trigger }) => {
        if (!trigger) return;

        const shouldToast = trigger.dataset.clipboardToast !== 'false';
        const toastMessage = trigger.dataset.clipboardToastMessage || 'Copied to clipboard';
        const toastVariant = trigger.dataset.clipboardToastVariant || 'success';

        if (shouldToast) {
            showToast(toastMessage, toastVariant);
        }

        const originalLabel = trigger.dataset.clipboardLabel ?? trigger.textContent ?? '';
        trigger.dataset.clipboardLabel = originalLabel;

        const successLabel = trigger.dataset.clipboardSuccessLabel || 'Copied!';
        trigger.textContent = successLabel;
        trigger.classList.add('is-copied');

        const resetDelay = Number.parseInt(trigger.dataset.clipboardReset ?? '2000', 10);
        if (Number.isFinite(resetDelay) && resetDelay >= 0) {
            const timeoutId = setTimeout(() => {
                trigger.textContent = trigger.dataset.clipboardLabel || originalLabel;
                trigger.classList.remove('is-copied');
                clipboardResetTimers.delete(trigger);
            }, resetDelay);
            clipboardResetTimers.set(trigger, timeoutId);
        }
    },
    onCopyError: (error, { trigger }) => {
        console.error('Clipboard copy failed', error);
        if (!trigger) return;

        const shouldToast = trigger.dataset.clipboardToast !== 'false';
        const message = trigger.dataset.clipboardErrorMessage || 'Unable to copy to clipboard. Copy manually.';
        if (shouldToast) {
        ...
        const errorLabel = trigger.dataset.clipboardErrorLabel || 'Retry';
        trigger.textContent = errorLabel;
        ...
        if (missing.length) {
            ...
            missing.forEach(tool => {
                ...
                if (installCommand) {
                    ...
                    action.addEventListener('click', assistantHandlers.handleInstallTool);
                } else if (docsLink) {
                ...
                if (installCommand) {
                    ...
                    copyButton.className = 'copy-chip';
                    copyButton.textContent = 'Copy';
                    copyButton.dataset.clipboard = 'command';
                    copyButton.dataset.clipboardText = installCommand;
                    copyButton.dataset.clipboardLabel = 'Copy';
                    copyButton.dataset.clipboardSuccessLabel = 'Copied!';
                    copyButton.dataset.clipboardReset = '2200';
                    copyButton.dataset.clipboardToastMessage = `Install command for ${formatToolLabel(tool)} copied.`;
                    copyButton.dataset.clipboardToastVariant = 'success';
                    copyButton.dataset.clipboardErrorLabel = 'Retry';
                    copyButton.dataset.clipboardErrorMessage = 'Unable to copy install command. Copy manually.';
                    copyButton.setAttribute('aria-label', `Copy install command for ${formatToolLabel(tool)}`);
