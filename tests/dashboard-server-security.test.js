...
test('dashboard observability history endpoint forwards since filters', async (t) => {
    const { baseUrl, origin, serverInstance, stop } = await createTestServer(t);

    let receivedOptions = null;
    serverInstance.observabilityExporter.getSnapshotHistory = (options) => {
        receivedOptions = options;
        return {
            history: [{ emittedAt: '2025-01-01T00:10:00.000Z' }],
    ...
    const response = await fetch(`${baseUrl}/api/observability/history?limit=3&since=2025-01-01T00%3A05%3A00Z`, {
        headers: {
            ...baseHeaders,
            Cookie: sessionCookie,
            Origin: origin
    ...
    assert.equal(body.overview.totalSnapshots, 1);
    assert.ok(receivedOptions);
    assert.equal(receivedOptions.limit, 3);
    assert.equal(receivedOptions.since, Date.parse('2025-01-01T00:05:00Z'));
