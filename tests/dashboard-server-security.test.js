...
test('dashboard observability stream emits fingerprint updates', async (t) => {
    ...
    if (latest.delta !== undefined) {
        assert.equal(typeof latest.delta, 'object');
    }
...
test('dashboard observability history endpoint returns bounded snapshots', async (t) => {
    ...
    assert.equal(body.overview.totalSnapshots, body.history.length);
...
test('dashboard observability incidents endpoint returns filtered incident report', async (t) => {
    const { baseUrl, origin, serverInstance, stop } = await createTestServer(t);
    ...
    serverInstance.observabilityExporter.history = sampleHistory;
    serverInstance.observabilityExporter.historyLimit = 20;

    const { sessionCookie } = await bootstrapSession(baseUrl, origin);

    const response = await fetch(`${baseUrl}/api/observability/incidents?status=failing&open=false`, {
        headers: {
            ...baseHeaders,
            Cookie: sessionCookie,
            Origin: origin
    ...
    assert.equal(body.matched.total, 1);
    assert.equal(body.matched.open, 0);
    assert.equal(body.records.length, 1);
    assert.equal(body.records[0].sink, 'webhook');
    assert.equal(body.records[0].status, 'failing');
