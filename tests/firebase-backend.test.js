...
const collection = (name) => {
  ...
  return {
    doc,
    async add(data) {
      const id = `mock-${adminMockData.adds.length + 1}`;
      const key = `${name}/${id}`;
      adminMockData.adds.push({ collection: name, data });
      adminMockData.docs.set(key, applyFieldValues({}, data));
      return { id };
    },
...
const telemetryService = await import('../functions/lib/telemetry-service.js');
...
describe('telemetry-service', () => {
  ...
  it('records deployment telemetry and updates project metadata', async () => {
    const result = await recordDeploymentEvent({
      ...
      metadata: { target: 'hosting', durationMs: 42000 },
      startedAt: '2024-05-01T10:00:00.000Z',
      completedAt: '2024-05-01T10:01:30.000Z',
      recordedAt: '2024-05-01T10:01:45.000Z'
    }, { adminOverride: firebaseAdminMock });

    expect(result.deploymentId).toMatch(/^mock-/);

    const stored = adminMockData.docs.get(`deploymentEvents/${result.deploymentId}`);
    expect(stored).toMatchObject({
      projectId: 'project-123',
      provider: 'firebase',
      environment: 'production',
    ...
    const projectDoc = adminMockData.docs.get('projects/project-123');
    expect(projectDoc.telemetry.lastDeployment).toMatchObject({
    ...
    expect(adminMockData.adds).toContainEqual(expect.objectContaining({
      collection: 'adminEvents',
      data: expect.objectContaining({
  ...
  it('lists deployments by recency and filters by provider', async () => {
    await recordDeploymentEvent({
      ...
      recordedAt: '2024-05-02T12:05:00.000Z'
    }, { adminOverride: firebaseAdminMock });
    ...
    await recordDeploymentEvent({
      ...
      recordedAt: '2024-05-01T09:05:00.000Z'
    }, { adminOverride: firebaseAdminMock });

    const timeline = await listDeploymentsForProject({
      projectId: 'project-123',
      limit: 5
    }, { adminOverride: firebaseAdminMock });

    expect(timeline.entries).toHaveLength(2);
    expect(timeline.entries[0].provider).toBe('firebase');
    expect(timeline.entries[0].recordedAt).toMatch(/^2024-05-02/);
    ...
    const filtered = await listDeploymentsForProject({
      projectId: 'project-123',
      provider: 'vercel'
    }, { adminOverride: firebaseAdminMock });

    expect(filtered.entries).toHaveLength(1);
    expect(filtered.entries[0].provider).toBe('vercel');
  });

  it('records GitHub insights and computes deltas between snapshots', async () => {
    await recordGitHubInsight({
      projectId: 'project-123',
      owner: 'domusgpt',
      repo: 'nimbus-guardian',
      stars: 120,
      forks: 14,
      openIssues: 6,
      openPullRequests: 2,
    ...
    await recordGitHubInsight({
      ...
      watchers: 92,
      defaultBranch: 'main',
      fetchedAt: '2024-05-02T09:30:00.000Z',
      metadata: { release: 'v1.2.0' }
    }, { adminOverride: firebaseAdminMock });

    const insights = await listGitHubInsightsForProject({
      projectId: 'project-123'
    }, { adminOverride: firebaseAdminMock });

    expect(insights.entries).toHaveLength(2);
    expect(insights.entries[0]).toMatchObject({
      stars: 125,
      forks: 16,
      openIssues: 5,
      openPullRequests: 3,
    ...
    const projectDoc = adminMockData.docs.get('projects/project-123');
    expect(projectDoc.github).toMatchObject({
      owner: 'domusgpt',
      repo: 'nimbus-guardian',
      stars: 125,
      forks: 16
    ...
    const adminEvents = adminMockData.adds.filter((entry) => entry.collection === 'adminEvents');
    expect(adminEvents).toEqual(expect.arrayContaining([
      expect.objectContaining({
        data: expect.objectContaining({
          type: 'GITHUB_INSIGHT_RECORDED'
        })
      })
    ]));
  });
