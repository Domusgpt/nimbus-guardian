...
test('ObservabilityExporter summarizes history overview metrics', () => {
    ...
    assert.ok(overview.sinks.incidents);
    assert.equal(overview.sinks.incidents.total, 2);
    assert.equal(overview.sinks.incidents.open || 0, 0);
    assert.equal(overview.sinks.incidents.byStatus.degraded, 1);
    assert.equal(overview.sinks.incidents.byStatus.failing, 1);
    assert.equal(overview.sinks.incidents.longestDurationMs, timestamps[2] - timestamps[1]);
    assert.ok(Array.isArray(overview.sinks.incidents.records));
    assert.equal(overview.sinks.incidents.records.length, 2);

    const diskIncident = overview.sinks.incidents.records.find((record) => record.sink === 'disk-log');
    assert.ok(diskIncident);
    assert.equal(diskIncident.status, 'degraded');
    assert.equal(diskIncident.startedAt, new Date(timestamps[1]).toISOString());
    assert.equal(diskIncident.endedAt, new Date(timestamps[2]).toISOString());
    assert.equal(diskIncident.open, false);
    assert.equal(diskIncident.durationMs, timestamps[2] - timestamps[1]);

    const webhookIncident = overview.sinks.incidents.records.find((record) => record.sink === 'webhook');
    assert.ok(webhookIncident);
    assert.equal(webhookIncident.status, 'failing');
    assert.equal(webhookIncident.startedAt, new Date(timestamps[1]).toISOString());
    assert.equal(webhookIncident.endedAt, new Date(timestamps[2]).toISOString());
    assert.equal(webhookIncident.open, false);
    ...
    assert.equal(limited.overview.sinks.incidents.total, 2);
});

test('ObservabilityExporter tracks ongoing sink incidents in overview analytics', () => {
    const snapshots = [
        {
            sinks: [
                { name: 'webhook', status: 'degraded', metrics: { attempts: 1 } }
            ]
        },
        {
            sinks: [
                { name: 'webhook', status: 'degraded', metrics: { attempts: 2 } }
            ]
        }
    ];
    ...
    const { overview } = exporter.getSnapshotHistory({ includeOverview: true });
    assert.ok(overview?.sinks?.incidents);
    assert.equal(overview.sinks.incidents.total, 1);
    assert.equal(overview.sinks.incidents.open, 1);
    assert.equal(overview.sinks.incidents.byStatus.degraded, 1);
    assert.equal(overview.sinks.incidents.openByStatus.degraded, 1);
    assert.equal(overview.sinks.incidents.longestDurationMs, timestamps[1] - timestamps[0]);

    assert.equal(overview.totalSnapshots, 2);

    const record = overview.sinks.incidents.records[0];
    assert.equal(record.sink, 'webhook');
    assert.equal(record.status, 'degraded');
    assert.equal(record.startedAt, new Date(timestamps[0]).toISOString());
    assert.equal(record.open, true);
    assert.equal(record.durationMs, timestamps[1] - timestamps[0]);
    assert.ok(!('endedAt' in record));
});
...
test('ObservabilityExporter incident reports support filtering and limits', () => {
    const exporter = new ObservabilityExporter({
        getSnapshot: () => ({}),
        debounceMs: 0,
        historyLimit: 20
    ...
    exporter.history = [
        {
            ...
            sinks: [
                ...
                { name: 'disk-log', status: 'healthy' }
            ]
        },
        {
            emittedAt: '2025-01-01T00:05:00.000Z',
            sinks: [
                { name: 'webhook', status: 'failing' },
                { name: 'disk-log', status: 'healthy' }
            ]
        },
        {
            emittedAt: '2025-01-01T00:10:00.000Z',
            sinks: [
                { name: 'webhook', status: 'failing' },
                { name: 'disk-log', status: 'healthy' }
            ]
        ...
        {
            emittedAt: '2025-01-01T00:15:00.000Z',
            sinks: [
                { name: 'webhook', status: 'healthy' },
                { name: 'disk-log', status: 'healthy' }
            ]
        ...
        {
            emittedAt: '2025-01-01T00:20:00.000Z',
            sinks: [
                { name: 'webhook', status: 'healthy' },
                { name: 'disk-log', status: 'degraded' }
            ]
        },
        {
            emittedAt: '2025-01-01T00:25:00.000Z',
            sinks: [
                { name: 'webhook', status: 'healthy' },
                { name: 'disk-log', status: 'healthy' }
            ]
        ...
        {
            emittedAt: '2025-01-01T00:30:00.000Z',
            sinks: [
                { name: 'webhook', status: 'degraded' },
                { name: 'disk-log', status: 'healthy' }
            ]
        }
    ];

    const report = exporter.getIncidentReport();

    assert.equal(report.total, 3);
    assert.equal(report.matched.total, 3);
    assert.equal(report.matched.open, 1);
    assert.ok(Array.isArray(report.records));
    assert.equal(report.records.length, 3);
    assert.equal(report.records[0].sink, 'webhook');
    assert.equal(report.records[0].status, 'degraded');
    assert.equal(report.records[0].open, true, 'most recent incident should remain open');

    const failingOnly = exporter.getIncidentReport({ statuses: ['failing'] });
    assert.equal(failingOnly.matched.total, 1);
    assert.equal(failingOnly.records.length, 1);
    assert.equal(failingOnly.records[0].sink, 'webhook');
    assert.equal(failingOnly.records[0].open, false);

    const openOnly = exporter.getIncidentReport({ open: true });
    assert.equal(openOnly.matched.total, 1);
    assert.equal(openOnly.records.length, 1);
    assert.equal(openOnly.records[0].sink, 'webhook');
    assert.equal(openOnly.records[0].status, 'degraded');

    const diskOnly = exporter.getIncidentReport({ sinks: ['disk-log'], open: false });
    assert.equal(diskOnly.matched.total, 1);
    assert.equal(diskOnly.records.length, 1);
    assert.equal(diskOnly.records[0].sink, 'disk-log');
    assert.equal(diskOnly.records[0].status, 'degraded');

    const limited = exporter.getIncidentReport({ limit: 1 });
    assert.equal(limited.records.length, 1);
    assert.equal(limited.matched.displayed, 1);
});
