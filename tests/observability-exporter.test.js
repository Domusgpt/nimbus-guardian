...
test('ObservabilityExporter filters history by since timestamp', () => {
    const snapshots = [
    ...
    const timestamps = [
        ...
        Date.parse('2025-01-01T00:04:00.000Z')
    ];
    ...
    const sinceIso = new Date(timestamps[1]).toISOString();
    const history = exporter.getSnapshotHistory({ since: sinceIso });
    assert.equal(history.length, 2);
    assert.equal(history[0].fingerprints.totalTracked, 2);
    assert.equal(history[1].fingerprints.totalTracked, 3);

    const limited = exporter.getSnapshotHistory({ since: sinceIso, limit: 1 });
    assert.equal(limited.length, 1);
    assert.equal(limited[0].fingerprints.totalTracked, 3);

    const overviewResult = exporter.getSnapshotHistory({ includeOverview: true, since: sinceIso });
    assert.equal(overviewResult.history.length, 2);
    assert.equal(overviewResult.overview.totalSnapshots, 2);
    assert.equal(overviewResult.overview.firstEmittedAt, sinceIso);

    const future = exporter.getSnapshotHistory({ since: timestamps[2] + 1 });
