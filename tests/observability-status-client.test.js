...
test('ObservabilityStatusClient forwards since filter when provided', async () => {
    const calls = [];
    const client = new ObservabilityStatusClient({
        url: 'http://localhost:3333/api/observability',
        fetchImpl: async (url) => {
            ...
            return {
                ok: true,
                status: 200,
                json: async () => ({ history: [], overview: null })
            };
        }
    });

    await client.fetchHistory({ since: '2025-01-01T12:34:56Z' });

    assert.equal(calls.length, 1);
    const parsed = new URL(calls[0]);
    assert.equal(parsed.searchParams.get('since'), '2025-01-01T12:34:56Z');
    assert.ok(!parsed.searchParams.has('limit'));
});

test('ObservabilityStatusClient normalizes numeric since filters', async () => {
    const calls = [];
    const client = new ObservabilityStatusClient({
        url: 'http://localhost:3333/api/observability',
        fetchImpl: async (url) => {
    ...
    await client.fetchHistory({ since: 1733856000123.987 });

    assert.equal(calls.length, 1);
    const parsed = new URL(calls[0]);
    assert.equal(parsed.searchParams.get('since'), String(Math.floor(1733856000123.987)));
});
